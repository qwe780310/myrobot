...
func init() {
	content, err := ioutil.ReadFile("F:\\workspace\\GoWorks\\src\\myproject\\first_server\\demo\\config.yaml")
	if err != nil {
...
func atMessageEventHandler(event *dto.WSPayload, data *dto.WSATMessageData) error {
	...
	if strings.HasSuffix(data.Content, "> tianqi") { //@机器人并输入 tianqi
	...
	if strings.Contains(data.Content, "> /天气") {
		...
		api.PostMessage(ctx, data.ChannelID, &dto.MessageToCreate{MsgID: data.ID, Ark: createArkForTemplate23(weatherData)})
	}
	return nil
}
...
func registerMsgPush() {
	...
	//这里表示每天15:53分发送消息
	timer.AddFunc("0 09 14 * * ?", activeMsgPush)
	timer.Start()
}
